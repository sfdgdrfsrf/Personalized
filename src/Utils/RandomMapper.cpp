...
size_t RandomMapper::map(size_t i) const {
    if (i >= m_size) {
        PZ_LOG_ERROR("RandomMapper::map index {:zu} out of range [0, {:zu})", i, m_size);
        return i;
    }
...
size_t RandomMapper::unmap(size_t j) const {
    if (j >= m_size) {
        PZ_LOG_ERROR("RandomMapper::unmap index {:zu} out of range [0, {:zu})", j, m_size);
        return j;
    }
...
std::vector<size_t> RandomMapper::fisherYatesShuffle(uint64_t seed, size_t n, int passes) {
    ...
    PZ_LOG_TRACE("Shuffle complete: perm[0..4] = [{:zu}, {:zu}, {:zu}, {:zu}, {:zu}]",
...
std::vector<size_t> RandomMapper::partialScramble(
    ...
    PZ_LOG_DEBUG("Selected {:zu}/{:zu} indices for scrambling ({:.1f}%)",
