/**
 * InventoryScrambleHook.cpp — Creative inventory scrambling. [v0.1.1]
 *
 * Hook strategies:
 *
...
void InventoryScrambleHook::applyShuffle() {
    ...
    PZ_LOG_INFO("Creative inventory shuffled: {:zu} items, {:d} passes",
...
void InventoryScrambleHook::install() {
    ...
    // ── Periodic re-shuffle (if enabled in config) ──
    if (cfg.inventoryRescrambleIntervalTicks > 0) {
        ...
        s_tickListener = bus.emplaceListener<ll::event::ServerTickEvent>(
            [interval](ll::event::ServerTickEvent&) {
                if (!SeedManager::instance().isInitialized()) return;

                ++s_tickCounter;
                if (s_tickCounter >= interval) {
                    s_tickCounter = 0;
    ...
    // Use a static flag and check on each tick until seeded
    static bool oneTimeShuffleDone = false;
    bus.emplaceListener<ll::event::ServerTickEvent>(
