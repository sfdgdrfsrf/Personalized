...
void TextureSwapHook::buildRemapTable() {
    ...
    PZ_LOG_INFO("Texture remap table built: {:zu} blocks, intensity {:.0f}%",
    ...
    if (cfg.verboseLogging && s_blockCount > 10) {
        for (size_t i = 0; i < 10 && i < s_blockCount; ++i) {
            PZ_LOG_DEBUG("  block[{:zu}] → block[{:zu}]", i, s_remapTable[i]);
        }
    }
}
...
LL_TYPE_INSTANCE_HOOK(
    ...
    for (size_t i = 0; i < s_blockCount; ++i) {
        ...
        PZ_LOG_TRACE("Swapped render: block[{:zu}] now renders as block[{:zu}]", i, targetIdx);
    }

    PZ_LOG_INFO("Texture swap applied to {:zu} blocks", s_blockCount);
}
*/
...
LL_TYPE_INSTANCE_HOOK(
    ...
    // size_t origIdx = 0; // resolve legacy → numeric ID
